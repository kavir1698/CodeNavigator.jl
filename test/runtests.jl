...
@testset "analyze_function_calls 1" begin
  ...
  fcall_in_random_nearby_agent = ["nearby_ids", "itsample", "abmrng", "sampling_with_condition_single", "Iterators.filter", "isnothing", "f", "StreamSampling.AlgR"]
  @test length(functions["random_nearby_agent"]) == length(fcall_in_random_nearby_agent) # all calls are included and no duplicates
  @test all([f in functions["random_nearby_agent"] for f in fcall_in_random_nearby_agent])
end

@testset "analyze_function_calls 2" begin
  functions = scan_julia_file(file2, save_to_file=false, create_diagram=false, include_external_functions=true)
